...
interface LoginResponse {
  refreshToken: string; // Este es el Token B (El que sirve para votar)
}

// --- Carga de Datos ---
const users = new SharedArray('users', function () {
  return JSON.parse(open('../data/users_poc_1_voto.json')) as User[];
});
...
export default function () {
  ...
  sleep(1); // Pequeña pausa de procesamiento
  ...
  const voteParams = {
    headers: {
      ...commonHeaders, // Reusamos headers comunes
      'Authorization': `Bearer ${sessionToken}`, // ¡USAMOS EL TOKEN B!
    },
  };

  const voteRes = http.post(voteUrl, votePayload, voteParams);

  const voteCheck = check(voteRes, {
