...
import exec from 'k6/execution'; // Importante para saber qué usuario nos toca
...
export default function () {
  ...
  if (uniqueIndex >= users.length) return;
  ...
  const voteRes = http.post(voteUrl, votePayload, {
    headers: { ...commonHeaders, 'Authorization': `Bearer ${sessionToken}` },
  });
